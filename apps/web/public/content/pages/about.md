# About / Profile

## ビジョン
誰もが「生きててよかった」と思える瞬間をmakeする。

## 関心領域
- HCI / 生理反応 / 主観的意味づけ
- “ストレス＝悪” に還元しない理解と介入
- 外在化インターフェースによる自己理解と行動変容

---

## 連絡先
- **Email**: [f.kotaro.0530@gmail.com](mailto:f.kotaro.0530@gmail.com)
- **X (Twitter)**: [@HATAKE55555](https://x.com/HATAKE55555)
- **GitHub**: [furukawa1020](https://github.com/furukawa1020)
- **Protopedia**: [hatake](https://protopedia.net/prototyper/hatake)

---

### メッセージ
理論・実証（メタレビュー／公開データ解析）と資金獲得・社会実装（臨床/現場導入）を両輪に、10年スケールで「意味づけの科学」の構築を目指しています。
「普通」から少しズレた人も、生きやすい社会へ。

---

## 受託開発・業務実務
- **製造業DX・ソフトウェア開発実務**: 大同工業等での標準化活動、評価プロセス、TPS（トヨタ生産方式）改善、品質工学、現場DXソフトウェア開発。
- **ハードウェア＆ソフトウェア プロトタイプ受託開発**: M5Stack / Arduino / 各種センサー / 組み込み / 駆動部などのハードウェア、および React / Next.js / FastAPI / AI連携などのソフトウェア双方における高速プロトタイピング受諾・納品。
- **お仕事のご相談**: お問い合わせは [Contact](/contact) より随時受け付けています。
